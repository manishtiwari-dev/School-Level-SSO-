...
if (!function_exists('getCompetition')) {
    # return system settings value
    function getCompetition($key, $default = null)
    {
        try {
            $examData = Cache::remember('exams', 86400, function () {
                return SSOExam::all();
            });

            $examData = $examData->where('key', $key)->first();

            return $examData == null ? $default : $examData->value;
        } catch (\Throwable $th) {
            return $default;
        }
    }
