<x-backend.app-layout>
    ...
    @section('title')
        {{ localize('Update Syllabus') }} {{ getSetting('title_separator') }} {{ getSetting('system_title') }}
    @endsection
        ...
        <div class="container mt-3">
                    ...
                    <div class="card tt-page-header">
                        <div class="card-body d-lg-flex align-items-center justify-content-lg-between">
                            <div class="tt-page-title">
                                <h2 class="h5 mb-lg-0">{{ localize('Update Syllabus') }}</h2>
                            </div>
            ...
            <div class="row mb-4 g-4">
                <div class="col-xl-9 order-2 order-md-2 order-lg-2 order-xl-1">
                    <form action="{{ route('admin.syllabus.update') }}" method="POST" class="pb-650" id="product-form">
                        @csrf
                        <input type="hidden" name="id" value="{{ $subList->id }}">
                        ...
                        <div class="card mb-4" id="section-1">
                            <div class="card-body">
                                        ...
                                        @if (!empty($examlist))
                                            @foreach ($examlist as $exam)
                                                <option value="{{ $exam->id }}"
                                                    {{ $exam->id == $subList->exam_id ? 'selected' : '' }}>
                                                    {{ $exam->name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                ...
                                <div class="mb-4">
                                    ...
                                    <select class="form-select  border border-2" name="class_id" id="class_name"
                                        required>
                                        <option value="">Select Class</option>
                                        @if (!empty($classlist))
                                            @foreach ($classlist as $class)
                                                <option value="{{ $class->id }}"
                                                    {{ $class->id == $subList->class_id ? 'selected' : '' }}>
                                                    {{ $class->name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea class="form-control" name="short_description" id="short_description" rows="4"
                                        placeholder="{{ localize('Type your short description') }}"> {{ $subList->short_description ?? '' }}</textarea>
                                </div>
                ...
                <div class="col-xl-3 order-1 order-md-1 order-lg-1 order-xl-2">
                    <div class="card tt-sticky-sidebar">
                        <div class="card-body">
                            <h5 class="mb-4">{{ localize('Syllabus Information') }}</h5>
                            <div class="tt-vertical-step">
                                <ul class="list-unstyled">
                                    <li>
                                        <a href="#section-1" class="active">{{ localize('Update Syllabus') }}</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
